...
public class GitHubController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUserByUsernamePollyFallback2(string userName, CancellationToken token)
    {
        var user = await _gitHubService.GetUserByUserNameAsyncFallbackPolly2(userName, token);
        return user is not null ? Ok(user) : NotFound();
    }
