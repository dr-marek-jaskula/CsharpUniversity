...
{
    class TasksAscynAwait2
    {
        #region Privite Members
        /// <summary>
        /// The event finished callback for the Thread event example
        /// </summary>
        private static event Action EventFinished = () => { }; //tworzymy pusty event
        ...
        public static void Program25()
        {
            Log("Hello World");
            //ASYNCHRONOUS
            //asynchronous is if u start something and dont wait while its happening. It litterally means to not occur at the same time.
            //this means not that our code returns early, but rather it doesnt sit there blocking the code while it waits (doesnt block the thread)

            //Issues with Threads
            //threads are asynchronous: as they naturally do something while the calling thread that made it doesnt wait for it. 
            //code example
            if (RunThreadExamples)
            {
                ...
                Thread.Sleep(1000);
                Console.WriteLine("End of the code example");
                Console.WriteLine("-----------------------------");
                #endregion
                ...
                //Block and wait
                blockingThread.Join(); //to jest pointless. Robisz 2 thready, które mogłyby zrobić to soma w jednym threadzie 
                Log("After blocking thread");
                Thread.Sleep(1000);
                Console.WriteLine("End of the blocking code example");
                Console.WriteLine("-----------------------------");
                #endregion
                ...
                Thread.Sleep(1000);
                Console.WriteLine("End of the polling code example");
                Console.WriteLine("-----------------------------");
                #endregion
                ...
                }; //dodajemy do stwrzonego eventu kolejną funkcję strzałkową.

                eventThread.Start();
                Log("After event thread");
                Thread.Sleep(1000);
                Console.WriteLine("End of the event code example");
                Console.WriteLine("-----------------------------");
                #endregion

                #region Event-based Wait Method 
                //[to samo co Event Based, ale trochę inaczej napisane]
                ...
                Thread.Sleep(1000);
                Console.WriteLine("End of the event method code example");
                Console.WriteLine("-----------------------------");
                #endregion
            ...
            //ta opcja jest preferowana przez AngelSix
            ...
            // This also means after ever await the next line is typiaclly on a new thread if there is no synchonization contex
            // ***************************************************************
            // An expeption is if you use "ConfigureAwait(false)" [na koncu Taska po kropce]. Then the SynchronizationContex is totally ignored and the resuming thread is treated as if there were no context
            //
            // Przy UI oczekujesz, że wrócisz do tego samego threada i tak jest:
            // Po skonczonym Tasku, wraca to thread 1, ze względu na synchronous context
            //
            // Resuming on the original thread via the synchronization contex is an epensive thing (takes time) and so if you choose to not care about resuming on the same thread and want to save time you can use "ConfigureAwait" to remove the overhead
            //*****************************************************************
            ...
            Console.ReadLine(); //po to bo async jest background process i zeby nie wywalało jak sie main thread konczy
        }
        ...
        private static void WebDownloadString(string url)
        {
            ...
            Log($"Donloaded {url}. {result.Substring(0, 10)}");
        }
        ...
        private static async Task WebDownloadStringAsync(string url)
        {
            ...
            string result = await webClient.DownloadStringTaskAsync(new Uri(url)); //to await robi nowy Thread, ale idzie dalej z kodem dopiero jak thread sie zakończy, czyli to zadanie które tutaj dano. Wraca do poprzedniego Thread
            // można zamiast new Uri(url) dać po prostu url. Jednakże wtedy trzeba zastąpić typ returna z Taska na void. Ponieważ dla newUri(url) jest tam return Task.
            //Task to "obietnica spełnienia poniższej linijki kodu po tym jak pójdzie to co jest await"
            // zatem await działa tak, że robi return Task, ale dopiero po wykonaniu wszystkich linijek w metodzie. Mozna tez rozumiec wtedy ze task jest jak void.
        ...
        /// <summary>
        /// Does some work asynchonously for somebody and return a result
        /// </summary>
        /// <param name="forWho">Who we are doing the wrok for</param>
        /// <returns></returns>
        private static async Task<string> DoWorkAndGetResultAsync(string forWho) //Task, który zwraca string jeszcze, generic type
        {
            Log($"Doing work for {forWho}");
